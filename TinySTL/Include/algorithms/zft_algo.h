...
typename iterator_traits<InputIterator>::difference_type
count(InputIterator first, InputIterator last, Predicate)
{
    typename iterator_traits<InputIterator>::difference_type n = 0;
    for(; first != last; ++first)
        if(pred(*first))
...
//unique stable
template <class ForwardIterator>
ForwardIterator unqiue(ForwardIterator first, ForwardIterator last)
{
                 ...
                 RandomAccessIterator last)
{
    if(first != last)
    {
        __introsort_loop(first, last, value_type(first), __lg(last - first) * 1);
...
void __introsort_loop(RandomAccessIterator first,
                      ...
                      Size depth_limit)
{
    while(last - first > __zft_threshold)
    {
        if(depth_limit == 0)
        {
            partital_sort(first, last, last);
            return;
        }
...
void __final_insertion_sort(RandomAccessIterator first,
                            RandomAccessIterator last)
{
    if(last - first > __zft_threshold)
    {
        __insertion_sort(first, first + __zft_threshold);
        __unguarded_insertion_sort(first + __zft_threshold, last);
    }
    else
                      ...
                      Pointer buffer, Distance buffer_size)
{
    ...
    else if(len2 <= buffer_size)
    {
        ...
        __merge_backward(first, middle, end_buffer, last);
    }
    else
    {
        ...
        BidirectionalIterator new_middle = __rotate_adaptive(first_cut,
                middle, second_cut, len1 - len11, len22, buffer, buffer_size);
