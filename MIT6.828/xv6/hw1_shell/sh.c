...
void runcmd(struct cmd *cmd)
{
  ...
  case ' ':
   ...
   // fprintf(stderr, "exec not implemented\n");
    ...
    {
        char cmdPath[10] = "/bin/";     //go to the /bin path to find the file
        strcat( cmdPath, ecmd->argv[0] );
        if( execv( cmdPath, ecmd->argv ) == -1 )
        {
            char cmdPath2[20] = "/usr/bin/";     //go to /usr/bin path
  ...
  case '|':
    ...
    if( fork1() == 0 )
    {
        ...
        close( p[0] );
        close( p[1] );
        runcmd( pcmd->right );   //right side reads from p[0]
    }
    ...
    if( fork1() == 0 )
    {
        ...
        close( p[1] );
        close( p[0] );
        runcmd( pcmd->left );   //left side writes to p[1]
    }
