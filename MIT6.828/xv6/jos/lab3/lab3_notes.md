...
`libmain`:我怀疑我是见鬼了，死活解决不了，不知道为啥，一句话的东西，我看了别人的，一毛一样，我的总是出现一直有缺页异常，甚至把别人的内存分配、进程等文件全部拷贝过来替换也是一样的，这到底是个啥情况，现在还搞不清楚，试试先跳过。  
### 缺页异常和内存分配  
内存保护是一个操作系统的重要特征，确保了在一个程序的bug不会影响其他进程或是操作系统。  
操作系统通常依赖硬件支持完成内存保护。OS保持硬件信息关于什么虚拟地址是有效的，什么是无效的。当一个车给序尝试接触一个无效的地址或者它没有权限的地址，处理器停止程序然后陷入到内核，内核有关于尝试的操作的信息。如果该错误是可以修补的，内核可以修补它并且让程序继续运行。如果错误是不可修补的，程序就不能继续，因为它将永远不能跨国引起错误的指令。  
作为一个可以修补的错误的例子，考虑一个自动扩增的栈。在许多系统中内核初始的分配一个页给栈，如果一个程序在获取栈之下的页的时候发生错误，内核会自动分配一页然后让程序继续。  
系统调用可以呈现一个有趣的内存分配问题。大多数系统调用接口让用户程序传递指针给内核，这些指针指向用户的要被读或者被写的缓冲区。内核然后在完成系统调用时解引用这些指针，这有两个问题：  
1. 一个在内核中的缺页异常可能比用户程序中的更严肃。如果在操纵自己的数据结构的时候内核发生了缺页异常，那就是内核的bug，并且缺页异常处理应当使内核崩溃。但是当内核解引用用户给它的指针而发生缺页异常时，它需要一个方法来记住任何的由解引用用户指针而引起的异常。  
2. 内核通常有比用户程序更多的内存权限。用户程序可能传递一个指针指向内核可以读或写而程序不能的内存给一个系统调用。内核必须小心不被欺骗。  
现在你将解决这两个问题，当一个程序传递给内核一个指针，内核将检查该地址是否是用户空间的，以及页表将允许内存分配。  
练习9：修改`kern/trap.c`来崩溃如果内核模式下发生了异常  
读在`kern/pmap.c`中的`user_mem_assert`，并完成`user_mem_check`.  
改变`kern/syscall.c`，检查系统调用参数。  
启动内核，运行`user/buggyhello`。进程不应该崩溃。  
最后，改变`kern/kdebug.c`中的`debuginfo_eip`来调用`user_mem_check`，在`usd`，`stabs`，`stabstr`中。  
练习10：引导内核，运行`user/evilhello`。进程应当被销毁，而内核不应当崩溃。
